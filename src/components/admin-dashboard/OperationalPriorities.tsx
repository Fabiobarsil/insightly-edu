...
const initialPriorities: PriorityItem[] = [
  { id: "1", icon: "ri-user-unfollow-line", iconClass: "bg-destructive/10 text-destructive", name: "Lucas Ferreira — 4ºC", desc: "Frequência crítica: 68% — abaixo do mínimo", status: "critico", resolved: false, students: [{ id: "mock-1", name: "Lucas Ferreira", issue: "Frequência 68% — abaixo do mínimo de 75%" }] },
  { id: "2", icon: "ri-file-warning-line", iconClass: "bg-warning/10 text-warning-foreground", name: "3 documentos pendentes", desc: "RG, Comprovante de residência e Histórico escolar", status: "pendente", resolved: false, students: [{ id: "mock-2", name: "Ana Clara Silva", issue: "RG pendente" }, { id: "mock-3", name: "Pedro Henrique", issue: "Comprovante de residência" }, { id: "mock-4", name: "Julia Santos", issue: "Histórico escolar" }] },
  ...
  { id: "4", icon: "ri-user-unfollow-line", iconClass: "bg-destructive/10 text-destructive", name: "Mariana Oliveira — 2ºB", desc: "Frequência crítica: 72% — risco de reprovação", status: "critico", resolved: false, students: [{ id: "mock-5", name: "Mariana Oliveira", issue: "Frequência 72% — risco de reprovação" }] },
  { id: "5", icon: "ri-file-warning-line", iconClass: "bg-warning/10 text-warning-foreground", name: "Cadastro incompleto", desc: "Gabriel Almeida — falta CPF e responsável", status: "pendente", resolved: false, students: [{ id: "mock-6", name: "Gabriel Almeida", issue: "Falta CPF e responsável cadastrado" }] },
];
        ...
        <div className="flex flex-col gap-2.5 flex-1 overflow-y-auto max-h-[360px] pr-1">
          ...
          {activePriorities.map((item) => (
            <div
              key={item.id}
              className={cn(
                "flex items-center gap-3 p-3 rounded-xl border border-border/40 border-l-[3px] hover:border-border transition-all duration-200 hover:shadow-sm group cursor-pointer",
                URGENCY[item.status]
              )}
